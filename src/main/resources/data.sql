...
-- EXPERIENCE
INSERT INTO experiences (id, position, company, duration, description, user_id) VALUES
  (1, 'Junior Java Developer', 'Netcare Sağlık Bilişim', 'Haz 2025 - Tem 2025 · 2 Ay (Remote)',
   'Netcare Sağlık Bilişim bünyesinde Junior Java Developer olarak görev aldım.
    Tıbbi görüntüleme (MR, Röntgen) sistemleri için Java tabanlı yazılım çözümleri geliştirdim,
    kullanıcı taleplerine uygun RESTful API’ler tasarladım ve mikroservis mimarisi üzerinde geliştirmeler gerçekleştirdim.
    Medikal yazılım çözümlerinde kalite ve performans odaklı geliştirmelerden sorumluydum.', 1),

  (2, 'Java Developer', 'HeAlthFintel', 'Mar 2025 - May 2025 · 3 Ay (Remote)',
   'HeAlthFintel ekibinde Java Developer olarak görev aldım.
    Kullanıcı ihtiyaçlarına uygun RESTful API’ler tasarladım, mikro hizmet tabanlı backend geliştirmeleri yaptım
    ve veri yönetimi süreçlerinde aktif rol üstlendim.
    Projede güvenli mimari, ölçeklenebilirlik ve performans optimizasyonlarına odaklandım.', 1),